...
- **One error shape across the whole SDK; five parallel ones deleted.** `platform.APIError`, `platform.StreamError`, `runtime.AppError`, `runtime.ErrorData` and `dispatch.DecodeError` are gone, along with the `runtime.Code*` constant list. A handler reports a failure by returning an `*errs.Error` - the type it declares, its detail, its declared members - and nothing else: no status, no title, no numeric code. Declarion fills the title from the declarations IT loaded, in the caller's language, which is why a sidecar needs no catalogue of its own and why one that shipped its own titles would freeze a language into a second place. The JSON-RPC number is now DERIVED from the type (`JSONRPCCodeFor`) rather than chosen per branch: it is a protocol artefact, and the identity a consumer branches on is `data.type`.

- **Non-2xx responses are parsed in ONE place, and an ingress failure stops wearing the platform's clothes.** Seven call sites each built their own error from a status and a body, one of them truncating at 500 characters. `errorFromResponse` keeps Declarion's own object untouched - type and members intact, filling only an ABSENT `status` from the response - and mints `transport.unreadable_response` when the body is not an error object, carrying the status, the path and a bounded body. "HTTP 502 from an ingress" and "the platform refused this" are different facts. `platform.StatusOf` and `platform.IsNotFound` read a failure without a type switch.

- **`ActionStream.Delivered()`** - how much of a streaming answer arrived before it ended, from the terminal event's own counts rather than counted locally: the two would disagree the moment a frame was dropped between them, and the server's count is the one that describes what it sent. A partially delivered answer is not a failed one, and a caller that cannot tell them apart either discards work that arrived or trusts an answer that never finished.

- **`platform.InvokeResult.Events`** - the declared synchronous chain the action ran, which this surface decoded and dropped on the floor. A Go consumer cannot act on a report it never receives; absence means "there was no such step", never "the step passed". Each entry carries the step's DECLARED position, the object a per-object step died on, how many it never reached, and the nested chain of its own.

- **The verifier's REASON no longer travels.** `VerifierError.Reason` is documented as internal telemetry that never reaches the caller, and it was going out as the JSON-RPC `message` - one hop from a public webhook response. It stays in the log line beside the refusal now; the wire carries the outcome type alone. **The three verifier wire codes are renamed** (`VERIFIER_REJECTED` → `verifier.rejected`, and its two siblings) in lockstep with declarion-core: a one-sided rename does not fail loudly, because an unrecognised code classifies as `unavailable` - correct as a fail-safe, and exactly why it would silently turn every rejection into a 503 the provider retries forever.

- **`errscheck.AssertNoUndeclaredTypes`** - the call-site gate as a helper an application calls once, against the catalogue its own loader produced in the same test process, so the gate reads the declarations that will ship rather than a fixture beside them.
